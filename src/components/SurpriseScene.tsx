...
export function SurpriseScene({ onBackToMenu, onNextScene }: SurpriseSceneProps) {
  ...
  useEffect(() => {
    if (currentLineIndex < 4) {
      setBGMVolume(BGM_VOLUME_LOW, 800);
    } else {
      setBGMVolume(BGM_VOLUME_NORMAL, 800);
    }
        ...
        <div className="relative flex items-end justify-center w-full max-w-sm h-76 sm:h-80">
          {isPetugas ? (
            ...
            <div
              ...
              className={`relative h-full flex items-end transition-all duration-300 z-30 scale-105 brightness-100 ${currentDialogue.animateStep
                  ? "animate-step-in"
                  : "animate-pixel-idle"
                }`}
            >
